...
def _passing_verification(candidate_id="c1", tests_run=("test_x",)):
...
def test_decide_rejects_compile_failure():
    ...
    decision = decide(_CANDIDATE, verification, mutation)
...
def test_decide_accepts_no_mutable_code_when_verbatim():
    ...
    decision = decide(_VERBATIM_CANDIDATE, verification, mutation)
...
def test_decide_escalates_when_sites_exist_but_all_uncovered():
    ...
    verification = _passing_verification()
    mutation = MutationScore(candidate_id="c1", mutants_total=0, mutants_killed=0, sites_total=2)
    decision = decide(_CANDIDATE, verification, mutation)
    assert decision.accepted is False
...
def _passing(candidate_id):
    return VerificationResult(
        candidate_id=candidate_id, compiled=True, tests_passed=True, tests_run=["test_x"]
    )
...
def test_decide_among_accepts_when_exactly_one_candidate_passes():
    ...
    verifications = [
        _passing("c-0"),
        VerificationResult(
            candidate_id="c-1", compiled=True, tests_passed=False, tests_failed=["t"]
        ),
    ]
    mutations = [
...
def test_decide_among_accepts_when_multiple_candidates_pass_with_identical_content():
    ...
    decision = decide_among(candidates, verifications, mutations)
    assert decision.accepted is True
    assert decision.candidate_id in ("c-0", "c-1")
