...
실행 기반 검증: 정적 도달성(BFS)은 상한선일 뿐이다 — 도달은 해도 이
sandbox 환경에서 못 도는 테스트(브라우저 launch가 필요한 e2e, 네트워크
붙는 smoke 등)가 섞여 나올 수 있다. `verify_relevant_tests`가 후보 테스트
파일들을 langs.effective_test_command로 **한 번의 배치 서브프로세스**로
실행해 초록인 것만 남긴다 — python 쪽 baseline이 "실행되고 통과한 테스트만"
인정하는 것과 같은 기준. 관련 테스트는 대개 다 초록이라 배치 한 번으로
끝나고(파일마다 러너를 새로 띄우던 N번 대비 절감), 배치가 실패할 때만
파일 단위로 격리해 어느 게 빨강인지 가려낸다. 이 결과도
`.weld_cache/test_exec.json`에 파일별 content-hash로
캐시해 callgraph.json과 같은 모델로 프로세스 간 재사용한다. 언어가 파일
단위 targeting을 지원 안 하면(langs.effective_test_command가 파일을 줘도
전체 스위트 명령과 똑같은 걸 돌려주는 경우 — make 기반 c/cpp, 실행 자체가
없는 go/rust/java 등) 검증은 구조적으로 자동 no-op된다: 언어 이름을
하드코딩해 분기하지 않고, "파일 지정 여부가 명령을 바꾸는가"만 보고 판단한다.
"""
...
from weld.langs import LanguageSpec, _LANGUAGES, effective_test_command
...
# C/C++는 pytest의 test_ 함수 같은 표준 단위테스트 컨벤션이 없어 함수 이름만으로는
# 테스트를 못 가른다(GoogleTest/Catch2 매크로는 아래 extractor가 따로 잡는다).
# 그 매크로도 안 쓰는 소박한 assert-in-main 스타일 테스트를 인식하려면 파일
# 이름/디렉터리 관례에 기댄다 — python impact.py의 _is_test_file과 같은 발상.
# 테스트 파일로 판정되면 그 안의 함수 정의를 is_test로 표시해 fallback 티어와
# green-verify가 그 파일을 실제로 돌려 검증하게 한다. effective_test_command의
# make TESTS= 규약과 짝을 이룬다(tests/test_mean.c 형태).
_TEST_FILE_DIR_NAMES = {"test", "tests"}
_TEST_FILE_STEM_SUFFIXES = ("_test", "_tests", "_unittest", "_spec")


def _is_test_file_by_name(rel_path: str) -> bool:
    parts = rel_path.split("/")
    stem = parts[-1].rsplit(".", 1)[0].lower()
    if stem.startswith("test_") or stem.startswith("test-") or stem == "test":
        return True
    if stem.endswith(_TEST_FILE_STEM_SUFFIXES):
        return True
    return bool(_TEST_FILE_DIR_NAMES.intersection(p.lower() for p in parts[:-1]))
...
def _extract_c(rel_path: str, source: bytes, root):
    ...
    # C는 함수 이름으로 테스트를 못 가르니 파일 관례로 판정한다. 테스트
    # 파일이면 그 안의 함수(대개 assert를 도는 main)를 테스트 노드로 인정한다.
    is_test_file = _is_test_file_by_name(rel_path)
    ...
    for node in all_nodes:
        ...
        nodes.append(GraphNode(qname, rel_path, start, end, is_test_file, name, None))
        qname_by_id[node.id] = qname
...
def _extract_cpp(rel_path: str, source: bytes, root):
    ...
    # GoogleTest/Catch2 매크로를 안 쓰는 소박한 assert-in-main 스타일 테스트도
    # 파일 관례로 인식한다(매크로는 아래에서 별도로 True 처리).
    is_test_file = _is_test_file_by_name(rel_path)
    ...
    for node in all_nodes:
        ...
        nodes.append(
            GraphNode(
                qname, rel_path, start, end, is_test_file, raw_name,
                _qname(rel_path, owner) if owner else None,
            )
        )
        qname_by_id[node.id] = qname
...
# ---------------------------------------------------------------------------
# 실행 기반 검증 — 정적 도달성은 상한선일 뿐이라, 실제로 이 sandbox
# 환경에서 돌아가고 초록인 테스트만 최종 채택한다. 모듈 docstring의
# "실행 기반 검증" 절 참고.
# ---------------------------------------------------------------------------

_EXEC_CACHE_VERSION = 1
_EXEC_TEST_TIMEOUT_SECONDS = 120

# (저장소 경로, rel_path) -> 통과 여부. 프로세스 생애 동안만 유지 — 디스크
# 캐시(_load_exec_cache/_save_exec_cache)가 프로세스 간 재사용을 맡는다.
_exec_memo: dict[tuple[str, str], bool | None] = {}


def _exec_cache_path(repo_root: Path) -> Path:
    return repo_root / ".weld_cache" / "test_exec.json"


def _load_exec_cache(repo_root: Path) -> dict[str, dict]:
    try:
        data = json.loads(_exec_cache_path(repo_root).read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError, UnicodeDecodeError):
        return {}
    if not isinstance(data, dict) or data.get("version") != _EXEC_CACHE_VERSION:
        return {}
    files = data.get("files")
    return files if isinstance(files, dict) else {}


def _save_exec_cache(repo_root: Path, files: dict[str, dict]) -> None:
    path = _exec_cache_path(repo_root)
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
            json.dumps({"version": _EXEC_CACHE_VERSION, "files": files}), encoding="utf-8"
        )
    except OSError:
        pass  # 캐시 저장 실패해도 기능엔 영향 없음 — 다음 실행이 다시 잰다.


def _supports_file_targeting(spec: LanguageSpec, repo_root: Path, rel_path: str) -> bool:
    """rel_path 하나로 좁힌 명령이 전체 스위트 명령과 실제로 다른가.

    언어 이름으로 분기하지 않는다 — langs.effective_test_command가 이
    rel_path에 대해 만들어주는 명령이 "전체 스위트를 그대로 위임"과
    구조적으로 같으면(같은 튜플), 실행 결과를 이 파일 하나의 탓으로 못
    돌린다(무관한 다른 파일이 원인일 수 있음) — 그런 언어는 검증 자체를
    꺼야 한다. make 기반 c/cpp, test_command가 아예 없는 go/rust/java가
    이 신호로 자연스럽게 걸러진다. 반대로 targeting을 지원하는 언어가
    새로 추가돼도(langs.py만 바뀌면) 여기는 손댈 필요가 없다."""
    targeted = effective_test_command(spec, repo_root, [rel_path])
    full = effective_test_command(spec, repo_root, None)
    return targeted is not None and targeted != full


def _run_tests(repo_root: Path, spec: LanguageSpec, rel_paths: list[str]) -> bool | None:
    """rel_paths를 한 번의 서브프로세스로 돌려(배치) 통과 여부를 잰다.

    반환: 명령이 exit 0이면 True(배치 전체 초록), 실패면 False(어느 파일
    탓인지는 호출부가 격리로 가려낸다), 실행 불가(명령 없음/러너 부재/프로세스
    오류)면 None(판정 보류). **타임아웃은 None이 아니라 False로 제외한다** —
    hang하는 e2e를 None으로 남기면 baseline이 다시 깨질 수 있어(fix_callgraph.md
    권고), 검증 목적상 '그린 아님'으로 본다. 파일 하나짜리 격리 실행에서도
    같은 규칙이라, hang하는 그 파일만 정확히 탈락한다."""
    command = effective_test_command(spec, repo_root, rel_paths)
    if command is None:
        return None
    try:
        result = subprocess.run(
            list(command), cwd=repo_root, capture_output=True, text=True,
            timeout=_EXEC_TEST_TIMEOUT_SECONDS,
        )
...
# _memoized_verdict가 "아직 안 잼"을 나타내는 센티넬 — bool/None과 구분한다.
_PENDING = object()


def _memoized_verdict(
    repo_root: Path, spec: LanguageSpec, rel_path: str, disk_cache: dict[str, dict]
) -> bool | None | object:
    """rel_path의 이미 알려진 검증 결과(bool 통과여부 / None 검증불가)나
    아직 안 잰 경우 _PENDING을 돌려준다. targeting 미지원 언어는 None
    (검증 자체를 건너뛰고 채택 — 지금까지의 동작 유지). content-hash로
    디스크 캐시(callgraph.json과 같은 모델)와 프로세스 생애 메모이즈를 함께
    본다 — 같은 실행에서 같은 파일이 여러 후보로 몰려도 한 번만 돈다."""
    if not _supports_file_targeting(spec, repo_root, rel_path):
        return None
    memo_key = (str(repo_root.resolve()), rel_path)
    if memo_key in _exec_memo:
        return _exec_memo[memo_key]
    try:
        content_hash = _content_hash((repo_root / rel_path).read_bytes())
    except OSError:
        _exec_memo[memo_key] = None
        return None
    entry = disk_cache.get(rel_path)
    if entry and entry.get("content_hash") == content_hash and "passed" in entry:
        _exec_memo[memo_key] = entry["passed"]
        return entry["passed"]
    return _PENDING
...
def _record_verdict(
    ...
    """방금 잰 결과를 메모이즈하고(항상), 확정 결과(True/False)면 disk_cache
    dict에도 반영한다. 디스크 저장 자체는 호출부가 배치 끝에 한 번만 한다."""
    _exec_memo[(str(repo_root.resolve()), rel_path)] = result
    if result is None:
        return
    try:
        content_hash = _content_hash((repo_root / rel_path).read_bytes())
        ...
        return
    disk_cache[rel_path] = {"content_hash": content_hash, "passed": result}


def _verify_test_files(
    repo_root: Path, spec: LanguageSpec, rel_paths: list[str]
) -> dict[str, bool | None]:
    """rel_paths 각각이 이 환경에서 돌아가고 초록인지를 {rel_path: 통과여부}로.

    캐시된(또는 targeting 미지원인) 파일은 건너뛰고, 아직 안 잰 파일들은
    **한 번의 배치 서브프로세스**로 돌린다 — 관련 테스트는 대개 다 초록이라
    흔한 경우는 서브프로세스 한 번으로 끝난다(파일마다 러너를 새로 띄우던 기존
    N번 대비 큰 절감). 배치가 실패(또는 타임아웃)하면 어느 파일 탓인지 알 수
    없으니 그때만 파일 단위로 격리해 실제 초록만 가려낸다."""
    disk_cache = _load_exec_cache(repo_root)
    outcomes: dict[str, bool | None] = {}
    pending: list[str] = []
    for rel_path in rel_paths:
        verdict = _memoized_verdict(repo_root, spec, rel_path, disk_cache)
        if verdict is _PENDING:
            pending.append(rel_path)
        else:
            outcomes[rel_path] = verdict  # type: ignore[assignment]

    if not pending:
        return outcomes

    batch = _run_tests(repo_root, spec, pending)
    if batch is True:
        results: dict[str, bool | None] = {p: True for p in pending}
    elif batch is None:
        # 러너 자체 부재 등 — 검증 불가. 전부 채택(None)하고 캐시엔 안 남긴다.
        results = {p: None for p in pending}
    else:
        # 배치가 빨강/타임아웃 — 파일 단위로 격리해 초록만 남긴다.
        results = {p: _run_tests(repo_root, spec, [p]) for p in pending}

    dirty = False
    for rel_path, result in results.items():
        outcomes[rel_path] = result
        _record_verdict(repo_root, spec, rel_path, result, disk_cache)
        if result is not None:
            dirty = True
    if dirty:
        _save_exec_cache(repo_root, disk_cache)

    return outcomes


def verify_relevant_tests(
    repo_root: Path, language: str, graph: CallGraph, qnames: set[str]
) -> set[str]:
    """정적 도달성(또는 폴백)으로 뽑힌 후보 중, 실제로 이 환경에서 돌아가고
    초록인 테스트의 파일만 남긴다. 검증은 파일 단위라 여러 후보가 같은 파일에
    몰리면 한 번만 실행되고, 서로 다른 파일들도 한 번의 배치로 함께 돈다.

    언어가 파일 단위 targeting을 지원 안 하면(_supports_file_targeting이
    구조적으로 감지) 검증 자체를 건너뛰고 후보를 그대로 반환한다 — 지금까지
    해오던 정적 도달성 그대로이지, 더 나빠지지 않는다."""
    spec = _SPEC_BY_NAME.get(language)
    if spec is None:
        return qnames

    file_by_qname: dict[str, str] = {}
    for qname in qnames:
        node = graph.nodes_by_qname.get(qname)
        if node is not None:
            file_by_qname[qname] = node.rel_path

    # 순서 보존 중복 제거로 파일별 한 번씩만 검증 대상에 넣는다.
    outcomes = _verify_test_files(
        repo_root, spec, list(dict.fromkeys(file_by_qname.values()))
    )

    # False(실제로 돌려보니 빨강)인 파일의 후보만 탈락. True/None은 채택.
    return {q for q, rel in file_by_qname.items() if outcomes.get(rel) is not False}
