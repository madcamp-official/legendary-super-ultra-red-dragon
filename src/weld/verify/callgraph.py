...
붙는 smoke 등)가 섞여 나올 수 있다. `verify_relevant_tests`가 후보 테스트를
langs.effective_test_command로 실제로 한 번 실행해(파일 단위) 초록인 것만
남긴다 — python 쪽 baseline이 "실행되고 통과한 테스트만" 인정하는 것과
같은 기준. 이 결과도 `.weld_cache/test_exec.json`에 파일별 content-hash로
...
def _run_test_file(repo_root: Path, spec: LanguageSpec, rel_path: str) -> bool | None:
    """rel_path 하나로 좁힌 명령을 실제로 돌려 통과 여부를 잰다.
    실행 불가(타임아웃/명령 없음/프로세스 오류)면 None(판정 보류)."""
    command = effective_test_command(spec, repo_root, [rel_path])
    if command is None:
        return None
    try:
...
def _verify_test_file(repo_root: Path, spec: LanguageSpec, rel_path: str) -> bool | None:
    """rel_path가 이 sandbox 환경에서 실제로 돌아가고 초록인지.

    content-hash로 디스크 캐시하고(callgraph.json과 같은 모델 —
    `.weld_cache/test_exec.json`), 프로세스 생애 동안은 메모이즈도 같이
    쓴다(같은 실행에서 여러 changed line이 같은 테스트 파일로 몰려도
    서브프로세스는 한 번만 뜬다). targeting 자체를 지원 안 하는 언어면
    None(검증 불가 — 필터링 안 하고 통과시킨다, 지금까지의 동작 유지)."""
    if not _supports_file_targeting(spec, repo_root, rel_path):
        return None

    repo_key = str(repo_root.resolve())
    memo_key = (repo_key, rel_path)
    if memo_key in _exec_memo:
        return _exec_memo[memo_key]

    try:
        source = (repo_root / rel_path).read_bytes()
    except OSError:
        _exec_memo[memo_key] = None
        return None
    content_hash = _content_hash(source)

    cache = _load_exec_cache(repo_root)
    entry = cache.get(rel_path)
    if entry and entry.get("content_hash") == content_hash and "passed" in entry:
        result = entry["passed"]
        _exec_memo[memo_key] = result
        return result

    result = _run_test_file(repo_root, spec, rel_path)
    _exec_memo[memo_key] = result
    if result is not None:
        updated = dict(cache)
        updated[rel_path] = {"content_hash": content_hash, "passed": result}
        _save_exec_cache(repo_root, updated)
    return result
...
def verify_relevant_tests(
    ...
    for qname in qnames:
        ...
        if node is None:
            continue
        outcome = _verify_test_file(repo_root, spec, node.rel_path)
        if outcome is not False:  # True(통과) 또는 None(검증 불가) — 채택
            verified.add(qname)
