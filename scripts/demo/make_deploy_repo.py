...
def readme(scns) -> str:
    lines = [
        ...
        "git clone <이 저장소>",
        "cd <저장소>",
        "./setup-weld.sh /path/to/weld     # weld 머지 드라이버 등록",
        "cp /path/to/weld/.env .           # LLM(.env) 설정 — [LLM 필요] 시나리오용",
        "```",
        "> 머지 드라이버 설정은 clone에 안 따라오므로 각자 한 번 실행해야 한다"
        " (`.gitattributes` 는 커밋되어 자동 적용).",
        "",
